...
type Lieferant struct {
	...
	Kundennummer    sql.NullString    `db:"Kundennummer"`
	Webseite        sql.NullString    `db:"Webseite"`
	Ansprechpartner []Ansprechpartner `db:"Anschprechpartner"`
}

func (d Database) GetLieferanten() ([]Lieferant, error) {

	lieferanten := []Lieferant{}
	query := `SELECT Lieferanten.*, 
		Anschprechpartner.id AS \"Ansprechpartner.Id\", 
		Anschprechpartner.Name AS \"Ansprechpartner.Name\", 
		Anschprechpartner.Telefon AS \"Ansprechpartner.Telefon\", 
		Anschprechpartner.Mobil AS \"Ansprechpartner.Mobil\", 
		Anschprechpartner.Mail AS \"Ansprechpartner.Mail\", 
		Anschprechpartner.lieferantenId AS \"Ansprechpartner.LieferantenId\" 
		FROM Lieferanten 
		JOIN Anschprechpartner 
		ON Lieferanten.id = Anschprechpartner.lieferantenId 
		ORDER BY Lieferanten.Firma ASC;
	`
	err := d.conn.Select(&lieferanten, query)
	if err != nil {
		fmt.Println(fmt.Errorf("LIEFERANTEN FEHLER: %w", err))
		return nil, err
	}
	fmt.Printf("LIEFERANTEN: %v", len(lieferanten))
...
func (d Database) GetLieferant(id string) (*Lieferant, error) {
	...
	lieferant := Lieferant{}
	err := d.conn.Get(&lieferant, "SELECT * FROM Lieferanten WHERE id = ? JOIN Anschprechpartner ON Lieferanten.id = Anschprechpartner.lieferantenId")
	if err != nil {
		return nil, err
	}
...
func (d Database) updateLieferant(params LieferantenParams, id string) (sql.Result, error) {
	...
	stmt, err := d.conn.Prepare("UPDATE Lieferanten SET Firma = ?, Kundennummer = ?, Webseite = ? WHERE id = ?;")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
...
func (d Database) createLieferant(params LieferantenParams) (sql.Result, error) {
	...
	stmt, err := d.conn.Prepare("INSERT INTO Lieferanten (id, Firma, Kundennummer, Webseite) VALUES (?, ?, ?, ?);")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
...
func (d Database) DeleteLieferant(id string) (sql.Result, error) {
	...
	stmt, err := d.conn.Prepare("DELETE FROM Lieferanten WHERE id = ?;")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
