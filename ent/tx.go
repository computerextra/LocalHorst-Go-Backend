...
func (tx *Tx) init() {
	tx.Ansprechpartner = NewAnsprechpartnerClient(tx.config)
	tx.Artikel = NewArtikelClient(tx.config)
	tx.Inventur = NewInventurClient(tx.config)
	tx.Lieferant = NewLieferantClient(tx.config)
	tx.Mitarbeiter = NewMitarbeiterClient(tx.config)
...
// applies a query, for example: Ansprechpartner.QueryXXX(), the query will be executed
