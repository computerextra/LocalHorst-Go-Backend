...
// QueryJahr chains the current query on the "Jahr" edge.
func (tq *TeamQuery) QueryJahr() *InventurQuery {
	query := (&InventurClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithJahr tells the query-builder to eager-load the nodes that are connected to
// the "Jahr" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TeamQuery) WithJahr(opts ...func(*InventurQuery)) *TeamQuery {
	query := (&InventurClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	...
	if query := tq.withJahr; query != nil {
		if err := tq.loadJahr(ctx, query, nodes, nil,
			func(n *Team, e *Inventur) { n.Edges.Jahr = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TeamQuery) loadJahr(ctx context.Context, query *InventurQuery, nodes []*Team, init func(*Team), assign func(*Team, *Inventur)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Team)
	for i := range nodes {
		if nodes[i].inventur_teams == nil {
			continue
		}
		fk := *nodes[i].inventur_teams
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "inventur_teams" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
