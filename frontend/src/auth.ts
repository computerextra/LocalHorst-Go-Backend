...
export const Login = async (mail: string, pass: string): Promise<ent.User> => {
  const res = (await ApiLogin(mail, pass)) as ent.User;
  return res;
};
...
export const GetUser = async (id: number): Promise<ent.User | undefined> => {
